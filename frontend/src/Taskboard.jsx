...
const Taskboard = ({ currentUser, currentUserRole = '' }) => {
  ...
  return (
    <div style={{ padding: '2rem', color: 'var(--text-primary)', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem' }}>
        ...
        {/* WL Tasks Column (For Recruiters or if they have WL tasks) */}
        {(currentUserRole.includes('recruiter') || currentUserRole.includes('coach') || wlTasks.length > 0 || historyTasks.length > 0) && (
        ...
        {/* Chaser & Admin Tasks Column (Conditionally Rendered) */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          
          {(currentUserRole.includes('chaser') || normalChaserTasks.length > 0) && (
          ...
          {(currentUserRole.includes('admin') || currentUserRole === 'super_admin' || currentUserRole === 'manager' || adminOfficerTasks.length > 0) && (
            <div style={{ background: 'var(--bg-surface)', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
          ...
          {(currentUserRole.includes('recruiter') || recruiterTasks.length > 0) && (
            <div style={{ background: 'var(--bg-surface)', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
              <h3 style={{ borderBottom: '2px solid #3b82f6', paddingBottom: '0.5rem', marginBottom: '1.5rem' }}>
                Recruiter Leads ({recruiterTasks.length})
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                {recruiterTasks.length === 0 && <p style={{ color: 'var(--text-secondary)' }}>No recruitment leads assigned.</p>}
              </div>
            </div>
          )}
